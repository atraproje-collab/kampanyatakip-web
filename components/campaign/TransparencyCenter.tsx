...
export function TransparencyCenter() {
  const { campaign, raised } = useCampaign();
  const { income, expenses } = campaign.transparency;
  const [tab, setTab] = useState<Tab>("income");

  const totalExpenses = expenses.reduce((s, e) => s + e.amount, 0);
  const netRemaining = raised - totalExpenses;
  const expenseRatio = raised > 0 ? (totalExpenses / raised) * 100 : 0;

  const stats = [
    {
      label: "Toplam Gelir",
      value: `₺${formatTRY(raised)}`,
      icon: TrendingUp,
      tone: "secondary" as const,
    },
    {
      label: "Toplam Gider",
      value: `₺${formatTRY(totalExpenses)}`,
      icon: TrendingDown,
      tone: "neutral" as const,
    },
    {
      label: "Net Kalan",
      value: `₺${formatTRY(netRemaining)}`,
      icon: Wallet,
        ...
        {stats.map(({ label, value, icon: Icon, tone, sub }) => {
          ...
          return (
            ...
            >
              ...
              {sub && (
                <p className="mt-1 text-[11px] text-on-surface-variant/85">
                  {sub}
                </p>
              )}
            </div>
            ...
            <table className="w-full text-[13.5px]">
              <thead className="bg-surface-container-low text-[11px] font-bold uppercase tracking-[0.12em] text-on-surface-variant">
                <tr>
                  ...
                  <th className="px-4 md:px-5 py-3 text-left hidden md:table-cell">Detay</th>
              ...
              <tbody className="divide-y divide-outline-variant">
                {income.map((row, i) => (
                  <tr key={i} className="hover:bg-surface-container-low/50 transition-colors">
                    ...
                    <td className="px-4 md:px-5 py-3.5 text-on-surface-variant hidden md:table-cell">
                      {row.details}
                    </td>
